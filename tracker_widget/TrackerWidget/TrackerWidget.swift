    ...
    private func fetchCurrentMonthExpenses() -> [ExpenseInfo] {
        ...
        do {
            ...
            for (index, expense) in allExpenses.enumerated() {
                print("\nExpense #\(index + 1):")
                print("   Amount: \(expense.value(forKey: "amount") ?? "nil")")
                print("   Date: \(expense.value(forKey: "date") ?? "nil")")
                if let category = expense.value(forKeyPath: "category.name") as? String {
...
struct LargeWidgetView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Two-column legend with 4 items per column
